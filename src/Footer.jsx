export default function Footer() {
  return (
    <div
      className="lg:flex md:flex md:flex-col xl:grid xl:grid-rows-[auto_auto_auto_auto]
 xl:grid-cols-3 xl:p-15 xl:pt-10"
    >
      <div className="col-span-3 xl:p-10 xl:pl-0 lg:pt-10 w-full h-full">
        <img
          src="picture/ableton-wordmark.svg"
          className="xl:w-45 lg:w-30 w-30 ml-10 mt-10 lg:mt-0 my-auto h-full"
          alt=""
        />
      </div>
      <div>
        <div className="p-10 pb-5 flex flex-col xl:text-xl font-light">
          <a href="#">
            Register Live, Push or Move{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            About Ableton{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            Jobs{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
        </div>
        <div className="px-10 flex flex-row gap-3">
          <svg
            role="img"
            aria-label="Facebook Ableton"
            viewBox="0 0 48 48"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-8"
          >
            <g clip-path="url(#clip0_4908_1133)">
              <path
                d="M48 24C48 10.7452 37.2548 3.05176e-05 24 3.05176e-05C10.7452 3.05176e-05 0 10.7452 0 24C0 35.2555 7.74863 44.6997 18.2022 47.2932V31.334H13.2532V24H18.2022V20.8398C18.2022 12.671 21.8989 8.88473 29.9188 8.88473C31.4392 8.88473 34.0628 9.18281 35.1361 9.48098V16.1294C34.5696 16.0697 33.5858 16.0399 32.3634 16.0399C28.4281 16.0399 26.9075 17.5306 26.9075 21.4063V24H34.7468L33.4001 31.334H26.9075V47.8242C38.791 46.3889 48 36.2705 48 24Z"
                fill="#0866FF"
              ></path>
              <path
                d="M33.4001 31.334L34.7469 24H26.9076V21.4062C26.9076 17.5305 28.428 16.0399 32.3634 16.0399C33.5857 16.0399 34.5695 16.0697 35.136 16.1293V9.48099C34.0628 9.18282 31.4392 8.88464 29.9187 8.88464C21.899 8.88464 18.2021 12.671 18.2021 20.8398V24H13.2531V31.334H18.2021V47.2932C20.0591 47.7539 22.0007 48 24 48C24.9843 48 25.9543 47.9393 26.9076 47.8242V31.334H33.4001Z"
                fill="white"
              ></path>
            </g>
            <defs>
              <clipPath id="clip0_4908_1133">
                <rect width="48" height="48" fill="white"></rect>
              </clipPath>
            </defs>
          </svg>
          <svg
            role="img"
            aria-label="X/Twitter Ableton"
            viewBox="0 0 48 48"
            xmlns="http://www.w3.org/2000/svg"
            fill="#000"
            className="w-8"
          >
            <path d="M36.6526 3.80786H43.3995L28.6594 20.6548L46 43.5798H32.4225L21.7881 29.6759L9.61989 43.5798H2.86886L18.6349 25.5601L2 3.80786H15.9222L25.5348 16.5165L36.6526 3.80786ZM34.2846 39.5414H38.0232L13.8908 7.63413H9.87892L34.2846 39.5414Z"></path>
          </svg>
          <svg
            role="img"
            aria-label="Youtube Ableton"
            viewBox="0 0 48 48"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-8"
          >
            <g clip-path="url(#clip0_4908_1126)">
              <path
                d="M47.044 12.3709C46.7726 11.3498 46.2378 10.4178 45.493 9.66825C44.7483 8.91872 43.8197 8.37794 42.8003 8.10003C39.0476 7.09094 24.0476 7.09094 24.0476 7.09094C24.0476 7.09094 9.04761 7.09094 5.29488 8.10003C4.27547 8.37794 3.34693 8.91872 2.60218 9.66825C1.85744 10.4178 1.32262 11.3498 1.05124 12.3709C0.0476075 16.14 0.0476074 24 0.0476074 24C0.0476074 24 0.0476075 31.86 1.05124 35.6291C1.32262 36.6503 1.85744 37.5823 2.60218 38.3318C3.34693 39.0813 4.27547 39.6221 5.29488 39.9C9.04761 40.9091 24.0476 40.9091 24.0476 40.9091C24.0476 40.9091 39.0476 40.9091 42.8003 39.9C43.8197 39.6221 44.7483 39.0813 45.493 38.3318C46.2378 37.5823 46.7726 36.6503 47.044 35.6291C48.0476 31.86 48.0476 24 48.0476 24C48.0476 24 48.0476 16.14 47.044 12.3709Z"
                fill="#FF0302"
              ></path>
              <path
                d="M19.1385 31.1373V16.8628L31.684 24.0001L19.1385 31.1373Z"
                fill="#FEFEFE"
              ></path>
            </g>
            <defs>
              <clipPath id="clip0_4908_1126">
                <rect width="48" height="48" fill="white"></rect>
              </clipPath>
            </defs>
          </svg>
          {/* eğer bulursan buraya instagram gelebilir */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            x="0px"
            y="0px"
            className="w-10"
            viewBox="0 0 48 48"
          >
            <radialGradient
              id="yOrnnhliCrdS2gy~4tD8ma_Xy10Jcu1L2Su_gr1"
              cx="19.38"
              cy="42.035"
              r="44.899"
              gradientUnits="userSpaceOnUse"
            >
              <stop offset="0" stop-color="#fd5"></stop>
              <stop offset=".328" stop-color="#ff543f"></stop>
              <stop offset=".348" stop-color="#fc5245"></stop>
              <stop offset=".504" stop-color="#e64771"></stop>
              <stop offset=".643" stop-color="#d53e91"></stop>
              <stop offset=".761" stop-color="#cc39a4"></stop>
              <stop offset=".841" stop-color="#c837ab"></stop>
            </radialGradient>
            <path
              fill="url(#yOrnnhliCrdS2gy~4tD8ma_Xy10Jcu1L2Su_gr1)"
              d="M34.017,41.99l-20,0.019c-4.4,0.004-8.003-3.592-8.008-7.992l-0.019-20	c-0.004-4.4,3.592-8.003,7.992-8.008l20-0.019c4.4-0.004,8.003,3.592,8.008,7.992l0.019,20	C42.014,38.383,38.417,41.986,34.017,41.99z"
            ></path>
            <radialGradient
              id="yOrnnhliCrdS2gy~4tD8mb_Xy10Jcu1L2Su_gr2"
              cx="11.786"
              cy="5.54"
              r="29.813"
              gradientTransform="matrix(1 0 0 .6663 0 1.849)"
              gradientUnits="userSpaceOnUse"
            >
              <stop offset="0" stop-color="#4168c9"></stop>
              <stop offset=".999" stop-color="#4168c9" stop-opacity="0"></stop>
            </radialGradient>
            <path
              fill="url(#yOrnnhliCrdS2gy~4tD8mb_Xy10Jcu1L2Su_gr2)"
              d="M34.017,41.99l-20,0.019c-4.4,0.004-8.003-3.592-8.008-7.992l-0.019-20	c-0.004-4.4,3.592-8.003,7.992-8.008l20-0.019c4.4-0.004,8.003,3.592,8.008,7.992l0.019,20	C42.014,38.383,38.417,41.986,34.017,41.99z"
            ></path>
            <path
              fill="#fff"
              d="M24,31c-3.859,0-7-3.14-7-7s3.141-7,7-7s7,3.14,7,7S27.859,31,24,31z M24,19c-2.757,0-5,2.243-5,5	s2.243,5,5,5s5-2.243,5-5S26.757,19,24,19z"
            ></path>
            <circle cx="31.5" cy="16.5" r="1.5" fill="#fff"></circle>
            <path
              fill="#fff"
              d="M30,37H18c-3.859,0-7-3.14-7-7V18c0-3.86,3.141-7,7-7h12c3.859,0,7,3.14,7,7v12	C37,33.86,33.859,37,30,37z M18,13c-2.757,0-5,2.243-5,5v12c0,2.757,2.243,5,5,5h12c2.757,0,5-2.243,5-5V18c0-2.757-2.243-5-5-5H18z"
            ></path>
          </svg>
          <svg
            role="img"
            aria-label="TikTok Ableton"
            viewBox="0 0 48 48"
            className="w-8"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M34.353 17.3269C37.4397 19.5413 41.2211 20.8442 45.3051 20.8442V12.9572C44.5322 12.9574 43.7613 12.8765 43.0051 12.7157V18.9239C38.9214 18.9239 35.1405 17.621 32.053 15.4068V31.5017C32.053 39.5533 25.5492 46.0798 17.5268 46.0798C14.5334 46.0798 11.7512 45.1716 9.44006 43.6141C12.0779 46.3208 15.7565 48 19.8263 48C27.8492 48 34.3534 41.4734 34.3534 33.4215V17.3269H34.353ZM37.1904 9.36999C35.6129 7.64045 34.5772 5.40535 34.353 2.93435V1.91992H32.1735C32.7221 5.06056 34.5934 7.74374 37.1904 9.36999ZM14.5142 37.4356C13.6329 36.2759 13.1566 34.8572 13.1587 33.3985C13.1587 29.716 16.1336 26.7302 19.8037 26.7302C20.4877 26.7301 21.1675 26.8352 21.8194 27.0427V18.9795C21.0576 18.8748 20.2888 18.8303 19.5203 18.8466V25.1226C18.868 24.915 18.1878 24.8096 17.5037 24.8103C13.8335 24.8103 10.8589 27.7957 10.8589 31.4787C10.8589 34.0828 12.3458 36.3373 14.5142 37.4356Z"
              fill="#FF004F"
            ></path>
            <path
              d="M32.0529 15.4066C35.1404 17.6208 38.9213 18.9237 43.005 18.9237V12.7156C40.7255 12.2283 38.7075 11.0328 37.1903 9.36999C34.5931 7.74358 32.722 5.0604 32.1733 1.91992H26.4482V33.4212C26.4352 37.0936 23.4655 40.0672 19.8032 40.0672C17.6451 40.0672 15.7279 39.0348 14.5136 37.4356C12.3454 36.3373 10.8585 34.0826 10.8585 31.4788C10.8585 27.7962 13.8331 24.8104 17.5032 24.8104C18.2064 24.8104 18.8842 24.9203 19.5199 25.1228V18.8468C11.6384 19.0102 5.2998 25.473 5.2998 33.4214C5.2998 37.3892 6.87827 40.9861 9.44013 43.6143C11.7513 45.1716 14.5335 46.08 17.5268 46.08C25.5494 46.08 32.0531 39.5531 32.0531 31.5017V15.4066H32.0529Z"
              fill="black"
            ></path>
            <path
              d="M43.0051 12.7156V11.037C40.9495 11.0401 38.9343 10.4624 37.1903 9.36987C38.7342 11.0661 40.7671 12.2357 43.0051 12.7156ZM32.1734 1.91997C32.1211 1.61982 32.0809 1.3177 32.053 1.01443V0H24.148V31.5016C24.1354 35.1735 21.1658 38.1471 17.5033 38.1471C16.428 38.1471 15.4128 37.891 14.5137 37.4358C15.7279 39.0349 17.6452 40.0671 19.8033 40.0671C23.4652 40.0671 26.4354 37.0938 26.4482 33.4214V1.91997H32.1734ZM19.5203 18.8468V17.0598C18.8598 16.9692 18.1938 16.9237 17.5271 16.924C9.50383 16.9239 3 23.4508 3 31.5016C3 36.5491 5.55612 40.9974 9.44034 43.614C6.87848 40.986 5.30002 37.3889 5.30002 33.4213C5.30002 25.473 11.6385 19.0102 19.5203 18.8468Z"
              fill="#00F2EA"
            ></path>
          </svg>
          <svg
            role="img"
            aria-label="Discord Ableton"
            viewBox="0 0 48 48"
            xmlns="http://www.w3.org/2000/svg"
            fill="#5865F2"
            className="w-8"
          >
            <path d="M40.6606 11.0467C37.5069 9.6029 34.1777 8.5786 30.7578 8C30.2898 8.83657 29.8664 9.6973 29.4893 10.5786C25.8465 10.0296 22.142 10.0296 18.4992 10.5786C18.1219 9.69739 17.6985 8.83668 17.2307 8C13.8087 8.58349 10.4772 9.61021 7.32038 11.0543C1.05329 20.3265 -0.645622 29.3685 0.203833 38.2821C3.874 40.9938 7.98197 43.0561 12.3492 44.3793C13.3325 43.0568 14.2027 41.6537 14.9504 40.1849C13.5302 39.6545 12.1595 39.0001 10.8541 38.2293C11.1977 37.9801 11.5337 37.7234 11.8584 37.4742C15.6567 39.2605 19.8025 40.1867 23.9999 40.1867C28.1974 40.1867 32.3431 39.2605 36.1415 37.4742C36.4699 37.7423 36.8059 37.999 37.1457 38.2293C35.8378 39.0014 34.4646 39.657 33.0419 40.1887C33.7887 41.6568 34.6589 43.0587 35.6431 44.3793C40.014 43.0614 44.1252 41.0001 47.796 38.2859C48.7927 27.949 46.0933 18.9901 40.6606 11.0467ZM16.0264 32.8003C13.6592 32.8003 11.7036 30.6521 11.7036 28.0094C11.7036 25.3666 13.5913 23.1996 16.0188 23.1996C18.4464 23.1996 20.3869 25.3666 20.3454 28.0094C20.3038 30.6521 18.4388 32.8003 16.0264 32.8003ZM31.9735 32.8003C29.6025 32.8003 27.6545 30.6521 27.6545 28.0094C27.6545 25.3666 29.5421 23.1996 31.9735 23.1996C34.4048 23.1996 36.3302 25.3666 36.2887 28.0094C36.2472 30.6521 34.3859 32.8003 31.9735 32.8003Z"></path>
          </svg>
        </div>
      </div>
      <div>
        <div className="p-10 xl:pl-0 pb-5 flex flex-col xl:text-xl font-light">
          <span className="font-semibold">Education</span>
          <a href="#">
            Offers for students and teachers{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            Ableton for the Classroom{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            Ableton for college and universities{" "}
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
        </div>
      </div>
      <div>
        <div className="p-10 pb-5 xl:pl-0 flex flex-col xl:text-xl font-light">
          {" "}
          <span className="font-semibold">Sign up to our newsletter</span>
          <div>
            Enter your email address to stay up to date with the latest offers,
            tutorials, downloads, surveys and more.
          </div>
          <div className="flex flex-row h-min items-center">
            <input
              type="text"
              className="bg-[#eee] my-5 p-2 w-[75%]"
              placeholder="Email Address"
            />
            <button className="bg-[#0000ff] font-semibold text-white w-[25%] h-fit p-2 justify-self-end">
              Sign up
            </button>
          </div>
        </div>
      </div>
      <div>
        <div className="p-10 pb-5 flex flex-col xl:text-xl font-light">
          <span className="font-semibold">Community</span>
          <a href="#">
            Find Ableton User Groups
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            Find Certified Training
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
          <a href="#">
            Become a Certified Trainer
            <svg
              width="20px"
              height="20px"
              stroke-width="1.5"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              color="#000000"
              className="inline justify-self-center items-center"
            >
              <path
                d="M9 6L15 12L9 18"
                stroke="#000000"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </a>
        </div>
      </div>
      <div className="col-span-2">
        <div className="p-10 xl:pl-0 pb-5 flex flex-col xl:text-xl font-light col-span-2">
          <span className="font-semibold">Language and Location</span>
          <div className="flex flex-row gap-1">
            <select className="bg-[#eeeeee] p-2 py-1 my-3 text-sm w-[25%] lg:w-[15%] font-normal">
              <option value="">English</option>
              <option value="">Deutsch</option>
              <option value="">Français</option>
              <option value="">日本語</option>
              <option value="">简体中文</option>
              <option value="">Español</option>
            </select>
            <select className="bg-[#eeeeee] p-2 py-1 my-3 text-sm w-[30%] lg:w-[20%] font-normal">
              <option value="">Turkey</option>
              <option value="">Germany</option>
            </select>
          </div>
        </div>
      </div>
      <div className="col-span-3">
        <div className="p-10 pb-5 flex flex-col gap-20 xl:text-xl font-normal col-span-3 justify-between">
          <div className="flex xl:flex-row flex-col xl:gap-5 gap-2">
            <a className="text-sm" href="#">
              Contact Us
            </a>
            <a className="text-sm" href="#">
              Press Resources
            </a>
            <a className="text-sm" href="#">
              Legal Info
            </a>
            <a className="text-sm" href="#">
              Corporate Information
            </a>
            <a className="text-sm" href="#">
              Privacy Policy
            </a>
            <a className="text-sm" href="#">
              Cookie Setings
            </a>
            <a className="text-sm" href="#">
              Imprint
            </a>
          </div>
          <div className="flex xl:flex-row gap-5 xl:text-xl font-normal col-span-3 items-center mr-auto flex-row-reverse">
            <span className="text-sm">Made in Berlin</span>
            <img
              src="picture/ableton-hallmark.svg"
              alt="Ableton Hallmark"
              className="w-12 h-auto"
            />
          </div>
        </div>
      </div>
    </div>
  );
}
